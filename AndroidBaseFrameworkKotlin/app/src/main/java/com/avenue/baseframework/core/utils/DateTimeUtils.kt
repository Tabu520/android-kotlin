...
object DateTimeUtils {
    ...
    fun formatDate(dateString: String?, locale: Locale?): Date? {
        var date: Date? = null
        dateString?.let {
            val iso8601Format = SimpleDateFormat(getDatePattern(it), locale)
            iso8601Format.timeZone = TimeZone.getTimeZone(timeZone)
            try {
                date = iso8601Format.parse(it.trim { char -> char <= ' ' })
    ...
    /**
     * Format a Date String to DATE_TIME_PATTERN_3 Date String
     *
     * @param date String Object
     * @return Date Object string representation
     */
    fun formatDatePattern_T(date: String): String? {
        var displayDate = ""
        val inputFormat = SimpleDateFormat(DateTimeFormat.DATE_TIME_PATTERN_3, Locale.getDefault())
        try {
            val index = date.indexOf('T') + 1
            val time = date.substring(index, index + 5)
            val dashboardDate = inputFormat.parse(date)
            displayDate = formatWithStyle(dashboardDate, DateTimeStyle.FULL)
                .toString() + " " + time
        } catch (e: ParseException) {
            e.printStackTrace()
        }
    ...
    fun formatWithMaximoPattern(date: Date?, pattern: String?): String? {
        val strDate = formatWithPattern(date, pattern, Locale.getDefault())
        val stringBuffer = StringBuffer(strDate)
        stringBuffer.insert(strDate.length - 2, ':')
        //return stringBuffer.toString().replace(":", "%3A").replace("+", "%2B");
    ...
    private fun getPatternForStyle(style: DateTimeStyle): String {
        val pattern: String = when (style) {
            DateTimeStyle.LONG -> "MMMM dd, yyyy"
            DateTimeStyle.MEDIUM -> "MMM dd, yyyy"
            DateTimeStyle.SHORT -> "MM/dd/yy"
    ...
    fun getTimeAgo(context: Context, date: Date?, style: DateTimeStyle): String {
        ...
        when {
            seconds <= 0 -> phrase = context.getString(R.string.time_ago_now)
            seconds < 45 -> {
    ...
    fun getCurrentHour(): Int {
        return getHourFromDate(Date())
    }

    fun getCurrentHourWithTimeShift(): Int {
        var hour = getHourFromDate(Date())
        if (hour > 0) {
            if (hour % 2 != 0) {
                hour = (hour + 1) % 24
            }
    ...
    fun convertShiftTimeToString(time: Long): String? {
        val dateFormat: DateFormat = SimpleDateFormat("HH:mm", Locale.getDefault())
        return dateFormat.format(convertShiftTimeToDate(time)!!)
    }

    fun convertShiftTimeToDate(time: Long): Date? {
        val shiftCalendar = Calendar.getInstance()
        shiftCalendar.timeInMillis = time
        val calendar = Calendar.getInstance()
        calendar[Calendar.HOUR_OF_DAY] = shiftCalendar[Calendar.HOUR_OF_DAY]
        calendar[Calendar.MINUTE] = shiftCalendar[Calendar.MINUTE]
        return calendar.time
    }

    fun getOneHourBetweenDates(startDate: Date?, endDate: Date): List<Date>? {
        ...
        while (calendar.time.before(endDate) || calendar.time == endDate) {
            ...
            if (endCalendar[Calendar.HOUR] == calendar[Calendar.HOUR]) {
    ...
    fun getHourOfDayFromListOfDates(listDates: List<Date?>): List<Int> {
        val hours: MutableList<Int> = ArrayList()
        for (date in listDates) {
            val calendar = Calendar.getInstance()
            calendar.time = date!!
            hours.add(calendar[Calendar.HOUR_OF_DAY])
        }
        return hours
    }

    fun getHourStringOfDayFromListOfDates(listDates: List<Date?>): List<String> {
        val hours: MutableList<String> = ArrayList()
        for (date in listDates) {
            val calendar = Calendar.getInstance()
            calendar.time = date!!
            hours.add(calendar[Calendar.HOUR_OF_DAY].toString())
        }
        return hours
    }
}